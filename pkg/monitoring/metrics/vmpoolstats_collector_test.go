...
var _ = Describe("VM Pool Stats Collector", func() {
	newVMPool := func(
		name, uid string, desired *int32, replicas, readyReplicas int32,
	...
	It("should emit info and replica gauges without putting replica counts on info labels", func() {
		pool := newVMPool("pool-1", "uid-1", ptr.To(int32(3)), 2, 1)
	...
	It("should default desired replicas to 1 when spec.replicas is unset", func() {
		pool := newVMPool("pool-1", "uid-1", nil, 0, 0)
	...
	It("should emit one series set per pool and drop deleted objects from the list", func() {
		live := newVMPool("pool-live", "uid-live", ptr.To(int32(2)), 2, 2)
		done := newVMPool("pool-done", "uid-done", ptr.To(int32(0)), 0, 0)
	...
	It("should set paused from spec.paused", func() {
		pool := newVMPool("pool-1", "uid-1", ptr.To(int32(1)), 1, 1)
	...
	DescribeTable("should set replica_failure from ReplicaFailure condition",
		func(status k8sv1.ConditionStatus, expected float64) {
			pool := newVMPool("pool-1", "uid-1", ptr.To(int32(1)), 0, 0)
			if status != "" {
