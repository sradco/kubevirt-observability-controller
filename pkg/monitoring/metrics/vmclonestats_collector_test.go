...
var _ = Describe("VM Clone Stats Collector", func() {
	newVMClone := func(
		name, uid, source, sourceKind, target string,
	...
	It("should emit one series per clone with matching uid, source, and target", func() {
		vmClone := newVMClone(
			"clone-1", "uid-1", "source-vm", "VirtualMachine", "target-vm",
	...
	It("should use status targetName when spec target is unset", func() {
		vmClone := newVMClone(
			"clone-1", "uid-1", "source-snap", "VirtualMachineSnapshot", "",
			clonev1.CreatingTargetVM, ptr.To("generated-target"),
		)
	...
	It("should emit one series per clone and drop deleted objects from the list", func() {
		live := newVMClone(
			"clone-live", "uid-live", "source-vm", "VirtualMachine", "target-a",
			clonev1.RestoreInProgress, nil,
		)
		done := newVMClone(
			"clone-done", "uid-done", "source-vm", "VirtualMachine", "target-b",
			clonev1.Succeeded, nil,
		)
	...
	It("should set snapshot_name and restore_name from status", func() {
		...
		vmClone.Status.SnapshotName = ptr.To("snap-1")
		vmClone.Status.RestoreName = ptr.To("restore-1")
