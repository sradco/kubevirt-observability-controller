...
var _ = Describe("VM Export Stats Collector", func() {
	newVMExport := func(
		name, uid, source, sourceKind string,
	...
	It("should emit one series per export with matching uid and source", func() {
		vmExport := newVMExport("export-1", "uid-1", "source-vm", "VirtualMachine", exportv1.Pending, true)
	...
	DescribeTable("should set phase from status",
		func(withStatus bool, phase exportv1.VirtualMachineExportPhase, expected string) {
			vmExport := newVMExport(
				"export-1", "uid-1", "source-pvc", "PersistentVolumeClaim", phase, withStatus,
			)
	...
	It("should emit one series per export and drop deleted objects from the list", func() {
		live := newVMExport("export-live", "uid-live", "source-vm", "VirtualMachine", exportv1.Ready, true)
		done := newVMExport(
			"export-done", "uid-done", "source-snap", "VirtualMachineSnapshot",
			exportv1.Terminated, true,
		)
	...
	It("should set vm from status.virtualMachineName for snapshot sources", func() {
		vmExport := newVMExport(
			"export-1", "uid-1", "source-snap", "VirtualMachineSnapshot",
			exportv1.Ready, true,
		)
		vmExport.Status.VirtualMachineName = ptr.To("source-vm")
	...
	It("should emit ttl expiration timestamp when status.ttlExpirationTime is set", func() {
		...
		vmExport := newVMExport("export-1", "uid-1", "source-vm", "VirtualMachine", exportv1.Ready, true)
	...
	It("should omit ttl expiration timestamp when status is nil", func() {
		vmExport := newVMExport("export-1", "uid-1", "source-vm", "VirtualMachine", exportv1.Pending, false)
