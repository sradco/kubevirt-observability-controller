...
var _ = Describe("VM Snapshot Stats Collector", func() {
	...
	It("should emit info and create-date with matching uid and source vm", func() {
		snapshot := newVMSnapshot("snap-1", "uid-1", "vm-1", snapshotv1.Succeeded, createdAt, ptr.To(true))
	...
	DescribeTable("should set phase from status", func(phase snapshotv1.VirtualMachineSnapshotPhase, expected string) {
		snapshot := newVMSnapshot("snap-1", "uid-1", "vm-1", phase, createdAt, nil)
	...
	It("should emit info with empty phase when status is nil", func() {
		snapshot := newVMSnapshot("snap-1", "uid-1", "vm-1", snapshotv1.InProgress, createdAt, ptr.To(true))
		snapshot.Status = nil
	...
	DescribeTable("should set ready_to_use from status", func(readyToUse *bool, expected string) {
		snapshot := newVMSnapshot("snap-1", "uid-1", "vm-1", snapshotv1.Succeeded, createdAt, readyToUse)

		results := reportVMSnapshotStats([]*snapshotv1.VirtualMachineSnapshot{snapshot})
		Expect(results[0].Labels[5]).To(Equal(expected))
	},
		Entry("nil readyToUse", nil, "false"),
		Entry("readyToUse false", ptr.To(false), "false"),
		Entry("readyToUse true", ptr.To(true), "true"),
	)

	It("should omit create-date when creationTimestamp is zero", func() {
		snapshot := newVMSnapshot("snap-1", "uid-1", "vm-1", snapshotv1.InProgress, metav1.Time{}, nil)
	...
	It("should emit one series set per snapshot and drop deleted objects from the list", func() {
		inProgress := newVMSnapshot("snap-live", "uid-live", "source-vm", snapshotv1.InProgress, createdAt, nil)
		failed := newVMSnapshot("snap-failed", "uid-failed", "source-vm", snapshotv1.Failed, createdAt, ptr.To(false))
